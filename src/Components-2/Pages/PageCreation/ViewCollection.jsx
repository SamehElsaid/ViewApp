...
export default function ViewCollection({ data, locale, onChange, readOnly, disabled, workflowId,pageId }) {
  ...
  const handleSubmit = e => {
    ...
    Object.keys(initialSendData).forEach(key => {
      ...
      if (initialSendData[keyData] !== null) {
        sendData[keyData] = initialSendData[keyData] ? initialSendData[keyData] : 3
      }
    })
    ...
    axiosPost(
      data.type_of_sumbit === 'collection' ? `generic-entities/${data.collectionName}/?pageId=${pageId}` : data.submitApi,
      locale,
      sendData,
      false,
      false,
        ...
        <form className={'w-[calc(100%)]'} onClick={() => setErrors(false)} onSubmit={handleSubmit}>
          ...
          >
            {sortedLoop.map(filed => (
              <div
                key={filed.id}
                className={`relative w-full ${filed.type === 'new_element' ? `s${filed.id}` : 'ss' + filed.id
