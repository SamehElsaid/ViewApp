import { Avatar, Card, CardContent, Chip, Typography, Button } from '@mui/material'
...
function AdminStart() {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    axiosPost(`internal-task/get`, locale, { pageNumber: paginationModel.page + 1, pageSize: 300 })
      .then(res => {
        if (res.status) {
          setData(res.data)
        }
  ...
  }, [locale, paginationModel.page, paginationModel.pageSize, startSearch, profile?.sub, profileLoading, refreshKey])
  ...
  const columns = [
    ...
    },
    {
      ...
      field: 'caseId',
      disableColumnMenu: true,
      headerName: messages.tasks.caseId,
      renderCell: ({ row }) => (
        <Typography variant='subtitle2' sx={{ fontWeight: 500, color: 'text.secondary' }}>
          {row.caseId}
        </Typography>
    ...
    },
    {
      ...
      field: 'typeName',
      disableColumnMenu: true,
      headerName: messages.tasks.taskType,
      renderCell: ({ row }) => (
        <Typography variant='subtitle2' sx={{ fontWeight: 500, color: 'text.secondary' }}>
...
function start() {
  return (
    <div>
      {process.env.APP_TYPE === "View as Admin" ? <AdminStart /> : <UserStart />}
    </div>
  )
}
...
export async function getServerSideProps(context) {
  ...
  if (process.env.APP_TYPE === "Form Builder") {
    return {
      notFound: true
    }
  }
